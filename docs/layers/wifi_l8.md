# WiFi Military Navigation

`wifi_l8` · Layer 8 · Group D — RF & Terrestrial

> WiFi as military nav landmarks + through-wall sensing

## Current status

**FABRICATES** — Invents a reading when nothing is connected, and invents a different one each time it is read.

Measured by constructing this layer with nothing attached and reading it three times: returned 13.08351, 80.27077, then a different position on the very next read, from the same (absent) input.

With simulation switched off it raises NotImplementedError.

## What this layer needs

**Not yet declared.** This layer has no `REQUIRES` declaration, so the repo cannot say what hardware or data would bring it to life. Adding one is part of cleaning the layer up — see [`FABRICATION_AUDIT.md`](../../FABRICATION_AUDIT.md).

## Details

| | |
|---|---|
| Class | `WiFiMilitaryNavLayer` |
| Source | [`upin/layers/rf/layers.py`](../../upin/layers/rf/layers.py) |
| Capabilities | POSITION, ENVIRONMENT |
| Accuracy rating | 0.60 |
| Novel | yes |

## How it works

```
Layer 8 — WiFi Signal Mapping as Military Navigation [NOVEL].

Adversary WiFi infrastructure as passive navigation landmarks.
Cannot be removed without disrupting their own operations.
Also provides WiFi radar through-wall sensing for structural
identification — critical for terminal guidance.
```

---

*Generated by `tools/generate_layer_docs.py`. Do not edit by hand — regenerate instead. The status above is measured at generation time, not asserted.*
