# SERF Atomic Spin Gyroscope

`serfgyro_l58` · Layer 58 · Group B — Inertial & Timing

> SERF atomic spin rotation sensing — chip-scale

## Current status

**FABRICATES** — Invents a reading when nothing is connected, and invents a different one each time it is read.

Measured by constructing this layer with nothing attached and reading it three times: returned heading 45.1 deg, then a different heading on the very next read, from the same (absent) input.

With simulation switched off it raises NotImplementedError.

## What this layer needs

**Not yet declared.** This layer has no `REQUIRES` declaration, so the repo cannot say what hardware or data would bring it to life. Adding one is part of cleaning the layer up — see [`FABRICATION_AUDIT.md`](../../FABRICATION_AUDIT.md).

## Details

| | |
|---|---|
| Class | `SERFGyroscopeLayer` |
| Source | [`upin/layers/inertial/layers.py`](../../upin/layers/inertial/layers.py) |
| Capabilities | HEADING |
| Accuracy rating | 0.88 |
| Novel | yes |

## How it works

```
Layer 58 — SERF Atomic Spin Gyroscope [NOVEL, UNDERWATER].

Spin-Exchange Relaxation Free gyroscope using alkali metal atoms.
Chip-scale (150cc) by Northrop Grumman.
Different mechanism from NMR — independent redundancy.
```

---

*Generated by `tools/generate_layer_docs.py`. Do not edit by hand — regenerate instead. The status above is measured at generation time, not asserted.*
